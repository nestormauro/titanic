...
class Predict:
    ...
    def set_predictions(self, train_file_path, test_file_path):
        if not str(self.algorithm).lower() in [
            x.lower() for x in settings.AVAILABLE_ALGORITHMS
        ]:
            raise ValueError(f"Invalid algorithm name: {self.algorithm}")

        model_path = (
            f"{os.getcwd()}/model/models/{str(self.algorithm).lower()}classifier.joblib"
        )
        model_exists = os.path.isfile(model_path)

        if not model_exists:
            raise FileExistsError(
                f"The trained model was not found here: {model_path} please run the training process before"
            )
